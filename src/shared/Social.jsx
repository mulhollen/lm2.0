...
import myspaceHover from '../img/myspace_hover.png'


const Social = () => {
  const [myspaceSrc, setMyspaceSrc] = useState(myspace);

  const hoverMyspace = inout => {
    if (inout === "in") {
      setMyspaceSrc(myspaceHover); 
    } else {
      setMyspaceSrc(myspace); 
    }
  }
  ...
  return (
    <section className="socials">
      <div>
        ...
        <Link to="/Myspace" className="mr-3 icon-link" onMouseEnter={() => hoverMyspace("in")} onMouseLeave={() => hoverMyspace("out")}>
          <img src={myspaceSrc} alt="myspace icon" />
        </Link>
      </div>
    </section>
  );
...
const Envelope = () => {
  return (
    <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="envelope-square" class="svg-inline--fa fa-envelope-square fa-w-14" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
